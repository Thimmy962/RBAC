...
class GroupTest(APITestCase):
    def setUp(self):
        ...
        self.retrieve_update_destroy_role = reverse("retrieve_update_destroy_role", kwargs = {"pk": 2})
        ...
        staff1 = Staff.objects.create_user(
            username = "Thimmy",
            password = "Uydnv1$1",
            is_staff=True
        )
        ...
        staff1.groups.add(self.view_staff)
        staff2.groups.add(self.view_grp)
        ...
        self.staff_1_access_token = token_response.data["access"]
        ...
        self.staff_2_access_token = token_response.data["access"]

    def test_manage_grp_with_authorized_cre_get_one_grp(self):
        # staff has view grp perm alone
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.staff_1_access_token}")
    ...
    def test_manage_stf_with_unauthorized_cre_get(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.staff_2_access_token}")
    ...
    def test_manage_grp_with_unauthorized_cre_get(self):
    ...
    def test_manage_grp_with_authorized_cre_get_one_grp(self):
        # staff has view grp perm alone
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.staff_2_access_token}")
        res = self.client.get(self.retrieve_update_destroy_role)
        self.assertEqual(res.status_code, 200)
